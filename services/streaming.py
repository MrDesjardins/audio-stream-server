...
def start_youtube_stream(youtube_video_id: str, skip_transcription: bool, broadcaster):
    ...
    if not audio_cache.check_file_exists(youtube_video_id):
        ...
        if config.transcription_enabled and not skip_transcription:
            ...
            ffmpeg_cmd = [
                "ffmpeg",
                "-err_detect", "ignore_err",  # Don't stop on minor errors
                "-fflags", "+genpts+igndts",   # Generate PTS, ignore DTS issues
                "-thread_queue_size", "1024",  # Increase input queue size
                "-i", "pipe:0",
                "-map", "0:a",                 # Map the audio stream
                "-c:a", "libmp3lame",          # MP3 encoder
                "-b:a", "192k",                # Constant bitrate for consistent quality
                "-ar", "48000",                # Sample rate
                "-ac", "2",                    # Stereo
                "-bufsize", "2048k",           # Larger buffer to prevent underruns
                "-f", "tee",
        ...
        ffmpeg_proc = subprocess.Popen(ffmpeg_cmd, stdin=yt_proc.stdout, stdout=subprocess.PIPE, bufsize=64*1024*1024)
